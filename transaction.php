<?php
    ...
    $stmt = $pdo->prepare("SELECT t.*, a.user_id FROM transactions t JOIN accounts a ON t.account_no=a.account_no WHERE t.id=:id LIMIT 1");
    $stmt->execute([':id'=>$id]);
    $t = $stmt->fetch();
    if (!$t) die('Not found');
    // ensure user owns the account or is admin
    if ($t['user_id'] != $_SESSION['user']['id'] && $_SESSION['user']['role'] !== 'admin') die('Access denied');
    ?>
    <!doctype html>
    <html><head><meta charset="utf-8"><title>Transaction Detail - SecureBank</title><link rel="stylesheet" href="assets/style.css"></head><body>
